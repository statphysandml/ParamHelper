...
* C++ source files:
  * `include/ParamHelper/ParamHelper.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/ParamHelper.cpp` is the main file that implements this library.
  * `app/ParamHelper_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/ParamHelper_t.cpp` contains the unit tests for the library.
